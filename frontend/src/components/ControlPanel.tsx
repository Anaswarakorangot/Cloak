  ...
  const doExport = async () => {
    ...
    if (exportFormat === 'pdf') {
      ...
      if ((isPdf || isImg) && document?.document_id) {
        try {
          const endpoint = isImg ? `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/export-image` : `${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/export-pdf`;
