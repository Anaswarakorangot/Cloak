...
export function UploadPage({ onAnalysisComplete }: Props) {
  ...
  const analyzeText = async (text: string, fileName: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/analyze-text`, {
        method: 'POST',
        headers: { 
  ...
  const analyzeFile = async (file: File) => {
    ...
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/analyze-upload`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const handleUseDemoDoc = () => {
    fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8000'}/api/analyze`, {
